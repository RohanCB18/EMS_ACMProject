...
export default function RegisterEventPage() {
    ...
    const loadForm = async () => {
        try {
            ...
            if (res.ok) {
                ...
                setError('Registration form not found. Please contact the organizers.');
            }
        } catch {
    ...
    const checkExistingRegistration = async () => {
        if (!user) return;
        try {
            const res = await fetch(`${API_URL}/api/registration/status/${user.uid}`);
            if (res.ok) {
                setSubmitted(true);
            }
        } catch {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const res = await fetch(`${API_URL}/api/registration/submit`, {
                method: 'POST',
            ...
            if (res.ok) {
                ...
                setError(err.detail || 'Failed to submit registration');
            }
        } catch {
